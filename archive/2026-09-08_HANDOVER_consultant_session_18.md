...
| **No `move_email_to_folder()` call sits inside either PER-ITEM email loop** | Loop membership computed from the syntax tree, 2026-09-08. Five calls. **Corrected 2026-09-08 by chat 19, and the correction is a wording fault of this session's: all five ARE inside a loop, being a per-message loop over `no_attachment_emails` or over `messages`. None is inside the loop over `embedded_images` or the loop over attachments, which is what 10f.33 needed.** ~~all outside~~ **The cause is worth knowing: this session's own AST script tagged only the two per-item loops and printed "no loop" for anything else, so it reported the absence of the two loops it was looking for as the absence of all loops.** Never true of this code before |
| **No bare `extractor.extract()` remains; four wrapped calls** | Syntax tree, 2026-09-08 |
| **`IntelliBooks\Inbox\` exists in neither codebase** | **The conclusion holds and the evidence line did not. Corrected 2026-09-09 by chat 19, and the wrong line was this session's.** ~~The literal `"Inbox"` is zero occurrences in `app.py`, `config.py`, `worker\` and `IntelliBooks-Desktop-v3.html`.~~ **That is true of a string literal exactly equal to `"Inbox"`, which is nought in the Python files, and FALSE of the substring: 13 occurrences across those four files**, counted as characters on 2026-09-08 with every matching line printed whole, all of them `Receipt Inbox` or `importToInbox`. **What no occurrence is, is `IntelliBooks\Inbox`, so the claim stands.** **Third correction to this table in one sitting and all three are one fault:** a tool of this session's answered a narrower question than the claim made from it, which is the trap section 11 now carries. **`Intellibills\Receipt Inbox\` was always there and the evidence line did not say so**, and that is what led to the folder being renamed to `IntelliBooks\Incoming\` before it existed, amendment 280 |
...
**And one that cost this session four wrong claims in two days. Learned 2026-09-08.** **A measuring tool of your own encodes the question you had when you wrote it.** Four instances: an AST script that tagged two loops and reported "no loop" for every other; a regex that needed a bold block to close and so missed sub-step 10f.34; `grep -c` read as an occurrence count; and a zero row count read as proof of a history. **In each case the tool was right about what it measured and the claim was about something else.** Print what the tool actually asked, beside the answer.
