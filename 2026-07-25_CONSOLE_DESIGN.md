...
| **Industry** | Zero or more per firm, one per `business_type`. **May ship with the product, may be uploaded by the firm, may not exist at all.** | The firm, by CSV upload. |
| **Client** | Created at client setup by copying **one** parent. | The firm, by editing, or by CSV upload. |
...
- Extend `chart_of_accounts_DRAFT.csv` with income, equity and remaining balance sheet accounts. ~~Not blocking; the 23 expense accounts cover the receipts module.~~ **Corrected 2026-08-03, amendment 90, after parsing the file rather than repeating this line.** It is 23 data rows in total, not 23 expense accounts: **20 `expenses`, 2 `assets` and 1 `liabilities`**. And it is blocking, per 18.10.
...
> **Not yet true, and deliberately so. Amendment 75, 2026-07-31.** Until 18.3's inbox handoff is built and passes the six-check acceptance test, Intellibills **continues** to write a filed receipt and its sidecar into `Clients\{client name}\Receipts\{tax year}\`, because that folder is today the only route a receipt has from capture into the books and 18.3's replacement does not exist. **`get_client_directory()`, `file_receipt()` and `make_enriched_sidecar()` are frozen for the duration and are out of scope for every 18.2b and 18.3 change.** The close condition, the frozen list and the reasoning are in section 0.5 of `C:\LastingImpact\receipt_capture\2026-07-31_PLAN_reset_and_restructure.md`. The simplification below is deferred, not cancelled.
