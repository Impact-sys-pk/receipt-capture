...
Section 18.2b abolishes both halves of that. ~~Intellibills never writes into `Clients\` at all, and~~ **Corrected 2026-08-21 by amendment 122: Intellibills does write there, and it is the only writer.** What is abolished is the folder's use as an interchange, not the write. The copy that goes there is image only with no data file, and it happens on a successful publish rather than on arrival. Section 18.3 replaces the interchange with a push into ~~`IntelliBooks\Inbox\`~~ **`IntelliBooks\Incoming\`, renamed 2026-09-09 by amendment 280 and named by firm setting F14**, a folder Desktop owns and drains. **Both halves exist from 2026-09-09: the pipeline publishes at stage 1 piece 3 and Desktop drains at stage 2.**

~~**`IntelliBooks\Inbox\` does not exist, nothing writes to it, and nothing drains it.** It appears in 18.2a's tree and nowhere in either codebase.~~ **Superseded 2026-09-09. It is `IntelliBooks\Incoming\` now, amendment 280, named by firm setting F14. The pipeline writes to it from commit `c8a453c`, stage 1 piece 3, and Desktop drains it from stage 2, both 2026-09-09.** **What remains true is that this interim's close condition is the six checks below and they have not been run: that is stage 3 and it is Paul's.**
...
**THE EXCEPTION IS CLOSED. 2026-09-09, amendment 298 of `2026-07-25_CONSOLE_DESIGN.md`. Check 1 passed in both clauses and stage 4 is complete: Intellibills writes into `Clients\` only on a successful publish and only when the firm's `client_copy_trigger` says `publish`, the copy is image only, and IntelliBooks Desktop no longer reads that folder for receipts at all. The paragraph below is kept as written because it is the record of what the interim was.** ~~**The exception.** Until the close condition in 0.5.1 is met, Intellibills continues to write a filed receipt and its sidecar into `Clients\{client name}\Receipts\{tax year}\`, and IntelliBooks Desktop continues to read them from there.~~ ~~This contradicts 18.2b, which says Intellibills never writes into `Clients\` at all.~~ **Corrected 2026-08-21 by amendment 134: it does not contradict 18.2b any more, and the sentence it cited has been struck.** Under amendment 122 Intellibills is the writer permanently, so what this interim defers is the **trigger**, being the move from writing on arrival to writing on a successful publish, and the sidecar that goes with the old route. **The contradiction is deliberate, it is dated, and it closes on a check rather than on a judgement.**
...
**ALL SIX HAVE PASSED. Checks 2 to 6 on 2026-09-09 as stage 3, amendment 291; check 1 in its two clauses the same day as stage 4's completion test, amendment 298. The results are in the design document, as this line asks.** ~~**Six checks. The exception closes when all six pass in one sitting, against one receipt, with the results written into the design document.** Not when 18.3 "feels done".~~

Each is written so it cannot be recorded as passed while the change is incomplete.

| # | Check | What it proves |
|---|---|---|
| 1 | **TWO CLAUSES, and both are run in one sitting. Reworded 2026-09-09 by amendment 292 of `2026-07-25_CONSOLE_DESIGN.md`, because the old wording could not pass on a working system after stage 4: 10f.11 makes Intellibills the only writer into `Clients\` and 10f.12's first trigger is on a successful publish, so the journey below writes one document there.** **Clause A: with F16 set to `never`, take a full listing of `Clients\` before the run and after it. A receipt travels from capture to the books and the two listings are identical.** **Clause B: with F16 set to `on successful publish`, the same journey adds EXACTLY ONE file to `Clients\`, being the document for that receipt under that client's own folder, and adds nothing else.** ~~Take a full listing of `Clients\` before the run and after it. **A receipt travels from capture to the books and the two listings are identical.**~~ | **Clause A proves the on-arrival write has stopped, which is what sub-step 10f.13 does. Clause B proves the new writer exists, is gated on the publish, and writes once.** A check that only inspects the inbox would pass while the pipeline still wrote both. ~~Intellibills wrote nothing into `Clients\`.~~ **That sentence was written on 2026-07-31 against 18.2b's original rule that Intellibills never writes into `Clients\` at all, and amendment 122 reversed it on 2026-08-21 on Paul's ruling.** **This check is stage 4's completion test, per amendment 290, and is not part of stage 3.** |
| 2 | The receipt appears as one item in ~~`IntelliBooks\Inbox\`~~ **`IntelliBooks\Incoming\`** after the pipeline runs and before Desktop is opened. **Renamed 2026-09-09 by amendment 280, before the folder existed, because `Intellibills\Receipt Inbox\` already exists and at stage 2 Desktop opens both.** **And the folder is named by firm setting F14 rather than fixed, so the check is against whatever `publish_destinations.intellibooks` holds, which is `Incoming` today.** | The push happened, and it happened into the folder Desktop owns. |
...
| 5 | Drain a second time without adding anything. **The books gain no second entry and ~~`IntelliBooks\Inbox\` is empty~~ `IntelliBooks\Incoming\` holds NONE OF THAT CLIENT'S ITEMS.** **Reworded 2026-09-09 by sub-step 10f.10: the inbox is one folder for every client per 10f.4, so "empty" cannot be the test while another client has something waiting, and "empty of that client's items" is.** | The drain removes what it took. Without this the handoff is a folder scan with a duplicate bug waiting in it. **This proof column is what settled the removal on 2026-09-09: the consultant session built the drain NOT to remove, on a safety argument, and this line plus 18.3's "a push with a queue in front of it" had already decided otherwise. Reversed the same hour, with the removal ordered after a successful save.** |
...
**High on section 0.1**, the interchange finding, because both halves were read from the file: `worker/filing.py:78` and `IntelliBooks-Desktop-v3.html:1281` and `:1288`. ~~**And high on the claim that `IntelliBooks\Inbox\` is unbuilt**, from grepping both codebases for it.~~ **Struck 2026-09-09 on two counts.** It is built, as `IntelliBooks\Incoming\`. **And the method was the narrower one:** a grep for a folder name is not a search of the codebase's behaviour, and the same shape of evidence line in `2026-09-08_HANDOVER_consultant_session_18.md` reported the literal `"Inbox"` as nought occurrences when the substring occurred 13 times, every one of them `Receipt Inbox` or `importToInbox`. **The conclusion was right both times and the evidence line was narrower than the claim both times.**
