...
41. Resolve in IntelliBooks Desktop, wait one poll, confirm the DB updated and the note moved to `processed\`. **Scoped 2026-07-28: both actions, filed and discarded, so two Review items and two OpenAI calls.** The discard branch has never run at all. Expect the filed receipt to read `ok` and the discarded one to read `discarded`, not `ok`: `discard_receipt()` sets `discarded` at `worker/resolution/service.py:805`. Both notes must reach `processed\`, and `Resolutions\failed\` must not exist afterwards.
