...
**114 plus 30 must equal 144, the highest number ever used.** If it does not, an item was removed without being recorded. Checked 2026-08-21. **Closed on 2026-08-21: items 1 to 14 entire, plus 26, 72, 79, 98, 99, 104, 139, 142, 143 and 144.** Items 53, 107, 110, 129, 136 and 137 were closed earlier. **Sections 1 and 2 are now both empty**, so nothing on this list blocks a scheduled step and nothing is waiting on Paul.
...
| 47 | Item 17 above, `categorisations_firm_vendors`. ~~which is the only one of these that is a leak rather than a limitation~~ **Corrected 2026-08-21 by amendment 131: it is one of two leaks, not the only one.** The other is item 143, `find_by_hash()` filtering on the hash alone, **and that one does not wait for the cloud**: it fires between two clients of one firm today |
