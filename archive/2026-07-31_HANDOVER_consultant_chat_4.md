...
**1. Plan the combined reset and restructure, 17.5a.** Six stages: stop the pipeline, back up, reset, restructure, change the code on both sides, then one clean cycle. The plan comes before any deletion. **Two precautions from 17.5 still bite and neither is optional:** keep the vendor mappings, which are 101 rows of real practice knowledge, and confirm `INBOX` is empty before touching `processed_attachments`, because anything sitting there is re-extracted at one OpenAI call each. ~~All of `Clients\Paul Keating\` and `PAUL-books.json` are confirmed disposable.~~ **Wrong and dangerous, corrected 2026-08-01. `Clients\Paul Keating\` stays.** It holds 37 files including engagement letters at its top level, `letter-of-engagement-ke001_*.pdf`, plus `Document Requests\` and `Misc\` from another tool. Those are real firm records. Only `Receipts\` and `Review\` inside it go, which is how 17.5 scoped it before I widened it in a summary. `PAUL-books.json` is disposable. **Supervised stage by stage, verified before and after each stage rather than once at the end. If a stage does not verify, stop there.**
