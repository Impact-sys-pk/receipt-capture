...
| F17 | **The path to the top client folder.** Today the folder name is the literal string `"Clients"`, hardcoded in seven places in `IntelliBooks-Desktop-v3.html` and in `config.py`'s `CLIENTS_ROOT` ~~at `config.py:33`~~ **line number dropped 2026-09-07 by amendment 247: it was wrong when written, since `:33` held the practice root, and `CLIENTS_ROOT` has since moved to `:74`** | Section 18.2b | Firm Settings, Intellibills Settings | No. One per firm | No | setting |
