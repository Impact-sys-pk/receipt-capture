...
Phase 1 uses only `scope='app_default'`. ~~`scope='default'`~~ **Renamed by amendment 95: the chart that governs a client is the firm default, and the app default is only what seeds it.** The firm, industry and client levels, the four import dispositions and the three statuses are specified in 13.1 and section 13 but not built.
...
Reserved so module 1 does not need reworking. `coa_accounts` (5.5) is created in phase 1 with `scope='app_default'` populated. ~~`scope='default'`~~ **Amendment 95.**
...
|---|---|---|
| **App default** | Ships with the product. One chart, the same for every firm. | Us. A firm never edits it. |
...
12. Load the app default chart into `coa_accounts` with `scope='app_default'` and `firm_id` NULL. **No file is named, because no app default chart exists yet.** ~~Load `chart_of_accounts_APP_DEFAULT_2026-08-03.csv`.~~ ~~Load `chart_of_accounts_DRAFT.csv` with `scope='default'`.~~ **Both struck 2026-08-03 on Paul's instruction, amendment 95.** The first named a file built without being asked for, since deleted. The second names a file that is not a chart of accounts at all. **This step is blocked until an agreed app default chart exists. Do not substitute a file for it.**
