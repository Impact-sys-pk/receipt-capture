...
- **The consultant session writes `IntelliBooks-Desktop-v3.html`. Standing instruction, Paul, 2026-09-06.** ~~It was written five times before this by exception, each on his instruction and each disclosed as a breach, and the handover of 2026-09-06 said in terms not to treat those as precedent and to ask.~~ **He was asked and this is the answer. It is no longer a breach and no longer needs disclosing case by case.** **What made it a standing question rather than five one-offs**: step 10e has eight outstanding sub-steps and six of them are Desktop work, so the rule was blocking the next step rather than an occasional task. **What does not change.** The Python pipeline stays with Claude Code and the consultant session does not write it. The occasions it did are recorded as exceptions and stay exceptions, most recently the one-line sidecar correction at amendment 236. **Paul still runs Desktop**, because it needs a real browser with folder access. **And the standard of evidence is the same standard, not a lighter one**: `node --check` on the whole script block with a negative control, the changed function driven directly over its cases, a `.bak-before-<change>` copy written first and proved byte-exact, and the written file staged back and md5 compared. **A single Cowork session now owns two of the three products, so the contract between them is no longer checked by two sessions disagreeing.** Amendment 242 of `2026-07-25_CONSOLE_DESIGN.md`.
...
- **Flag, do not fix.** Something wrong that the task did not ask about gets reported, not repaired. This has surfaced more real defects than any other single rule. **Extended 2026-09-05 on Paul's instruction, and the extension is narrow: flag it, and if it is small and obviously right, say so and offer to do it in the same reply.** The flag still happens and Paul still decides, so nothing is repaired on a session's own judgement mid-task, which is what this rule exists to stop. **What changes is that a small, agreed fix does not become an item on a list to be briefed later**, which is his instruction of 2026-08-24 under "Three sessions, and none of them can see the others" applied to findings as well as to agreed work: a scheduled small change costs a round trip to schedule, one to brief and one to report, and he is the only channel between the sessions. **The offer is part of the flag, not a separate turn.** Judge "small and obviously right" by whether the fix can be stated in one sentence and checked in one command; anything needing a decision is flagged and left, however small the edit would be.
...
- **A mutation is anchored to one place, and it prints its own diff as proof it changed one place. Added 2026-09-08, from three incidents in two days.** On 2026-09-07 a mutation script used `str.replace()`, which has no count, against two byte-identical blocks; it changed both, the next mutation found nothing left to change and reported "MUTATION DID NOT APPLY", and **it was visible only because the first run failed a test that did not fit the mutation that was supposed to have been made.** **A mutation that changes more than it says it does overstates what the suite caught**, which is the one thing a mutation exists to measure. So: anchor on something unique, assert the anchor matches exactly once, and print the unified diff beside the result rather than describing it.
- **The harness refuses a mutation whose anchor matches more than once, and that refusal has already earned its place twice. Added 2026-09-08.** It was written the day after the incident above and stopped the same fault on 2026-09-08, when an anchor was a guard line with sixteen spaces of indentation and **the other email path holds the identical line at twenty**, so the shorter string is a substring of the longer one and `str.count()` saw two. **The two email loops in `app.py` are close to line-for-line copies, so any anchor inside one must carry something the other lacks.** A comment is usually the only unique thing available.
- **Enumerate calls from the syntax tree, not by grepping, and compute the loop bound rather than eyeballing it. Added 2026-09-08.** A grep for a call name returns it from comments, docstrings and strings as well as from code, and on this project the prose is always there because superseded wording is kept beside every correction. Walking the tree also gives each call's **enclosing loop or function**, which is what several of this week's findings turned on: `move_email_to_folder()` inside a loop and outside it are two different behaviours, and the difference was measured from indentation rather than read. **This is the enumerate-the-set rule applied to a call graph rather than to a list of files.**
...
**Commit before a run whose `pipeline_version` matters, and expect the startup warning until you do.** `config.check_git_status_on_startup()`, called from `app.py` reports the state of the working tree at that moment, and `pipeline_version` is the git short hash from `config.py:153`. **So a run started on a dirty tree records a version that does not describe the code that ran**, and every receipt from that run carries it. The warning does not block, and nobody has asked for it to.
...
**The lock is `C:\Intellibills\pipeline.lock`, outside OneDrive. Moved there 2026-09-07 by amendment 248, commit `bac4292`, because process state belongs on the unsynced side of 18.2a with the database and the logs.** ~~`Intellibills\pipeline.lock`~~ **The old path is orphaned and Paul deletes it once.** **A leftover lock is normal and is not a fault. Do not raise it.** Paul starts the pipeline on demand and closes the console window, which is a `CTRL_CLOSE_EVENT` and does not run `release_lock()`, so the lock outlives every session. ~~and `acquire_lock()` clears it at the next start.~~ **That clause was the reassurance and is now the defect, struck 2026-09-07 by amendment 248.** `acquire_lock()` clears the lock at the next start **whether or not the process it names is alive**, because `_is_process_running()` cannot see a process outside its own tree: `signal.CTRL_C_EVENT` is `0` on Windows, so `os.kill(pid, 0)` is the console-event branch and raises `OSError [WinError 87]` for anything else, which is swallowed as `False`. **67 starts in five weeks, 63 stale removals, 0 refusals ever**, counted over the whole of `C:\Intellibills\logs\run.log`. ~~A fix is briefed as `PROMPT_claude_code_2026-09-07_lock_guard_fix.md`; until it lands, two pipelines can run at once and nothing stops them.~~ **FIXED 2026-09-07 by amendment 249, commit `e6fc821`. `acquire_lock()` now clears a lock only when the process it names has gone or its pid has been reused, and refuses when that process is genuinely alive.** `_is_process_running()` asks `OpenProcess` and `GetExitCodeProcess` rather than `os.kill(pid, 0)`, and the `started_at` the lock has always carried is compared against the process's creation time so a recycled pid cannot block a start. **`release_lock()` deletes only a lock naming its own pid.** **The first refusal in 71 starts across five weeks appeared at 10:39:15 on 2026-09-07.** **What Paul will see change**: running `IntelliBooks.bat` while a pipeline is already up now opens a console window that prints `Another pipeline process is already running` and exits, instead of quietly starting a second one. ~~The launcher's own guard is still broken and is a separate change.~~ **Corrected 2026-09-07 by amendment 250, twice over.** **The consultant session asserted three times that the launcher's `tasklist` guard is broken and never tested it.** What is established is that 16 starts happened on 2026-09-06 in clusters of three and four inside minutes, and that Paul runs the launcher repeatedly and leaves the console window open; **why the guard does not stop that is not established.** **And the launcher is not Paul's to fix**: `IntelliBooks.bat` sits in `IntelliBooks\App\` beside `IntelliBooks-Desktop-v3.html`, which the consultant session writes. **The diagnosis is moot in any case.** The pipeline now refuses on its own, so a second launch opens a window that prints the refusal and exits, which is cosmetic. **The launcher is being rewritten to read `C:\Intellibills\pipeline.lock` instead of guessing with `tasklist`.** **This was raised twice as a defect, as outstanding items 26 and 104**, and the second time it was reasoned about as "either a run has held the lock for twenty hours or the lock is stale for the second time in two days". Neither was right. The answer is written here rather than only in a closed item, because a session that sees the file on disk is not reading the closed items.

### Section 16 states every status twice, so check them against each other

Added 2026-09-14. **`2026-07-25_BUILD_STATUS.md` carries a step's status in the table near the top
AND at the head of that step's own paragraph.** Paul reads section 16 and nothing else, so the word
in the paragraph is where he actually sees it and it is not being removed. **What was missing is
anybody checking the two agree.** On 2026-09-14 seventeen paragraphs read OUTSTANDING under a table
row reading BUILT, and two steps had a row and no paragraph at all: one day's work updated the table
and left the rest. Step 10av had already been scheduled twice for the same reason, having looked
unbuilt.

**Run `py check_build_status.py` from the repository root before committing any change to section
16.** It compares the 80 table rows against the paragraph heads, prints every disagreement with its
line number, and exits 1. It takes about a second. Sub-steps like 10d.1 are deliberately not checked:
they have no table row, so their paragraph is the only record.

**It found its own author's omission on the first run**, a step marked BUILT in the table an hour
earlier and left OUTSTANDING in its paragraph by the session writing the check.
...
### Do not cite a line number in `config.py`

Added 2026-09-07. **Name the constant or the function instead.** `config.py` gained `_required_root()` on 2026-09-06 and everything from the roots downwards moved by about thirty lines: `PRACTICE_ROOT` 33 to 68, `UNSYNCED_ROOT` 37 to 69, `CLIENTS_ROOT` 42 to 74, `RESOLUTIONS_DIR` 96 to 128, and the `mkdir` block 129 to 161. **56 `config.py:NN` citations sit across `CLAUDE.md`, `2026-07-25_CONSOLE_DESIGN.md` and `2026-08-20_LIST_settings_firm_and_client.md`**, counted on 2026-09-07, and almost all of them are now wrong. **They are not being retrofitted**, because the same thing will happen at the next refactor and a document full of refreshed numbers is a document that looks checked. **A name does not move.** The ones inside the amendment record are dated history and stay as written; live text gets the name. **This is the same fault as `app.py:775`**, which says a lock is acquired at line 282 and it is acquired at 681.

### ~~Four~~ ~~Five~~ ~~Six~~ **Seven** traps that cost hours

**Six of the seven are live and the fourth is closed by code, struck below with its history kept. The seventh moved here on 2026-09-06 from the project instructions, which used to carry a second copy of this list; that copy is retired, because two copies of one list drift and that one had four when this had six.**
...
- ~~**Never import `config.py` from the Linux sandbox.**~~ **CLOSED 2026-09-06 by the code FOR THE LINUX SANDBOX ONLY, and NARROWED 2026-09-09 because that wording read as closed everywhere and it bit the same day.** **On Windows an `import config` outside pytest still reads `.env` and still runs the `mkdir` block against the LIVE roots.** On 2026-09-09 at 09:56 Claude Code imported it to check a setting against the live firm record and created `C:\Users\PDK7\OneDrive - Intellitax Accounting Limited\IntelliBooks\Incoming\`, which is the folder 18.2a specifies and was still a write outside the repository nobody had asked for. **It disclosed it, and the folder is kept because `INTELLIBOOKS_PUBLISH_DIR` is one of the six mkdir targets and the next pipeline start would make it anyway.** **So the live rule is: never `import config` to read a value. Read the constant out of the file, or run it under pytest, where `tests/live_paths.py` redirects both roots before the import.** `python3 -m py_compile config.py` remains the safe check and needs no environment. **The Linux half is genuinely closed and the reasoning below is still worth reading.** `INTELLIBILLS_PRACTICE_ROOT` and `INTELLIBILLS_UNSYNCED_ROOT` are now required and must be absolute, checked by `_required_root()`, which is defined above the `mkdir` block and therefore runs before it. ~~`PRACTICE_ROOT` and `INTELLIBILLS_UNSYNCED_ROOT` ... at `config.py:38-69`, 92 lines above the `mkdir` block.~~ **Corrected 2026-09-07: the variable was renamed to `INTELLIBILLS_PRACTICE_ROOT` hours after this was written, and the line numbers are dropped rather than refreshed, per the rule below.** **The Python constant is still `config.PRACTICE_ROOT`; only the environment variable carries the prefix.** **A Windows path string has no leading separator on Linux, so `Path(...).is_absolute()` is `False` there**, which means an import from the sandbox now raises `RuntimeError` and creates nothing, whether or not `.env` is present. Verified with `PurePosixPath` against the real `.env` values by Claude Code, and independently here. **`python3 -m py_compile config.py` is still the cheaper check and needs no environment.** **Reading a constant's value still needs Windows**, and now also needs the two variables set. Commit `f52675b`, amendment 245 of `2026-07-25_CONSOLE_DESIGN.md`. **The history below is what the trap was.** Added 2026-08-03 after finding the folder it made on 29 July. `config.py:117-121` calls `mkdir(parents=True, exist_ok=True)` on five paths at import, and `PRACTICE_ROOT` and `UNSYNCED_ROOT` default to Windows path strings at `:33-37`. **Those two were `ONEDRIVE_ROOT` and `LOCAL_ROOT` until sub-step 10d.21 renamed them on 2026-09-03. The trap is unchanged; only the names are.** **A backslash is an ordinary filename character on Linux**, so those strings become relative folder names and the mkdir block builds them inside the repository. The one it built on 29 July is **no longer at the repository root**, checked 2026-09-03, and nothing records when it went. It was:
  ...
  **What makes it worse than untidy is that git cannot see it.** It is not gitignored, but git does not track empty directories, so `git status` has been silent about it for five days and would stay silent. The moment anything writes a file inside it, it becomes untracked and trips the clean-tree warning `config.check_git_status_on_startup()` raises with a path nobody will recognise. **Anything in the pipeline that needs a config value from the sandbox: read the constant out of the file, do not import the module.** Removal needs Paul, because the sandbox can delete neither a file nor a directory in a mounted folder.

- **Exclude `.history\` from any repository-wide search. Added 2026-09-06.** It is VS Code's local history, it is gitignored, and it holds a dated copy of every file you have edited. **An enumeration of `vendor_key` and `vendor_code` across the repository returned 1,133 lines with it and 205 without**, so more than four fifths of the answer was old copies of the same five files. **A search that includes it does not merely return noise; it reports names that no longer exist anywhere live**, which is the opposite of what an enumeration is for. Gitignored is not the same as invisible to `grep`, `rg` or `glob`, and none of them read `.gitignore` unless told to.

- **A database read through the Cowork file bridge is not evidence. Added 2026-09-06.** A consultant session with no shell reaches `C:\Intellibills\db\receipts.db` by staging a copy, and **the bridge decides whether to re-send a file by its modification time. SQLite in write-ahead mode adds rows without that time moving.** On 2026-09-06 the staged copy said 14 receipts and stopped at 08:55 while the live file held 16, and the session spent an exchange trying to reconcile a learned row against receipts that were there all along. **Three rules follow.** One, **never assert a count or a row's absence from a staged database**; have Paul run the query and paste the output, or write the answer to a NEW file, which the bridge has no cached time for and therefore reads correctly. Two, **the same doubt applies to any file a running process writes without closing**, not only to SQLite. Three, **this affects nothing but the session's own reads**: IntelliBooks Desktop and the pipeline open their files directly and never touch the bridge.
- **A Cowork session may or may not have a shell on Paul's machine, and it is not constant between sessions. Moved here 2026-09-06 from the project instructions, which held it and this file did not.** A session that assumes it has one writes instructions for Paul to run instead of doing the work. **Establish it rather than assume it.** **With no shell, every read is `device_stage_files` and every write is `SendUserFile` then `device_commit_files`, and every write is read back and md5 compared.** **Git state can still be read exactly**, by staging `.git\HEAD`, `.git\refs\`, `.git\index` and the loose objects, which works because this repository has no pack files; `.git\logs\HEAD` is the reflog and gives the commit sequence in one file. **That reads the tracked side exactly and cannot see untracked files at all**, so list the folder immediately before predicting them.

**And one retired on 2026-09-06, recorded so it is not reinstated from an old document.** ~~Do not add a duplicate `client_id` check to `clients.csv`: one client may legitimately have two rows differing only in the email column, so the test is whether the other columns match, not whether the id repeats.~~ **That was true of `clients.csv`, which carried one `email` column and one row per client, so a client with two addresses needed two rows sharing an id. Step 10d replaced it.** Read on 2026-09-06: `clients.csv.superseded-2026-08-20` holds seven rows and a single `email` column; the live `Intellibills\clients.json` holds five records, each with an `emails` **list**, and no repeated `client_id`. **One record per client now, so a uniqueness check on `client_id` is valid rather than forbidden.**
